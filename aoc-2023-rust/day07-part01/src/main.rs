...
fn main() {
    ...
    let hand_map = compute_map(&ranks, &primes);
    ...
    for line_res in io::stdin().lock().lines() {
        ...
        let bid = u64::from_str_radix(temp[1], 10).unwrap();

        let mut hash = 1;
        for card in temp[0].chars() {
...
fn compute_map(ranks: &[char; 13], primes: &[u64; 13]) -> HashMap<u64, HandType> {
...
fn compute_hash(primes: &[u64; 13], i: usize, j: usize, k: usize, l: usize, m: usize) -> u64 {
